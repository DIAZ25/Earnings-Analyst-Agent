...
│   ├── ingestion/
│   │   ├── __init__.py
│   │   ├── parser.py              # PDF → text, HTML → text (BeautifulSoup)
│   │   ├── chunker.py             # Table-aware chunker (keeps tables atomic)
│   │   ├── embedder.py            # Sentence-transformers wrapper
│   │   ├── vector_store.py        # FAISS index per (ticker, doc_type)
│   │   └── edgar.py               # SEC EDGAR auto-downloader
│   │
│   ├── agents/
│   │   ├── __init__.py
│   │   ├── graph.py               # LangGraph state machine — wires all nodes
│   │   ├── supervisor.py          # Query classifier + state init (no LLM)
│   │   ├── financials_rag.py      # RAG retrieval + draft briefing (LLM)
│   │   ├── relevance_grader.py    # Chunk relevance filter (LLM)
│   │   ├── comparator.py          # Beat/miss/inline arithmetic (NO LLM)
│   │   ├── sentiment.py           # Sentiment scoring (rule-based + LLM)
│   │   ├── figure_grader.py       # Figure grounding audit (LLM)
│   │   ├── reflection.py          # Draft critique + query revision (LLM)
│   │   └── final_node.py          # Markdown briefing assembly (no LLM)
│   │
│   ├── evaluation/
│   │   ├── __init__.py
│   │   └── harness.py             # Accuracy eval against ground-truth dataset
│   │
│   └── observability/
│       ├── __init__.py
│       └── langfuse_client.py     # LangFuse tracing wrapper (no-op if unconfigured)
│
├── scripts/
│   └── ingest_company.py          # CLI: auto-download SEC filings or ingest local files
│
├── data/
│   ├── guidance_cache/            # JSON prior-guidance files per ticker
│   │   └── AAPL_example.json      # Example guidance structure
│   └── vector_stores/             # FAISS indexes (git-ignored, generated at runtime)
│
├── tests/
│   ├── __init__.py
│   ├── test_ingestion.py          # Chunker + parser unit tests
│   ├── test_agents.py             # Agent node unit tests (no API calls)
│   └── test_api.py                # FastAPI integration tests (LLM calls mocked)
│
